...
export default function SummaryCards({ totalIncome, totalExpense, balance, pct, incomeCount, expenseCount }) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <Card label="Renda Total" sub={`${incomeCount} fonte${incomeCount !== 1 ? 's' : ''}`}>
        <span className="text-gray-900">{formatBRL(totalIncome)}</span>
      </Card>

      <Card label="Despesas Total" sub={`${expenseCount} item${expenseCount !== 1 ? 's' : ''}`}>
...
function Card({ label, sub, children }) {
  return (
    <div className="bg-white border border-gray-200 rounded-xl px-5 py-4 shadow-sm hover:-translate-y-0.5 hover:shadow-md transition-all duration-150">
      ...
      <p className="text-2xl font-bold font-mono leading-tight">{children}</p>
